...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['serie_id'])) {
    ...
    if ($serie_id) {
        try {
            ...
            // Obtener capítulos
            $stmt_cap = $pdo->prepare("SELECT * FROM capitulos WHERE nombre_id = ? ORDER BY id ASC"); // Añadir orden
...
// --- CAMBIO PRINCIPAL AQUÍ ---
    ...
    $sql = "SELECT s.id, s.nombre, s.poster
            FROM series s
            INNER JOIN favoritos f ON s.id = f.serieid
            WHERE f.usuarioid = ?
            ORDER BY s.nombre ASC";

    // Ejecuta la consulta de forma segura
    $stmt_fav_series = $pdo->prepare($sql);
    $stmt_fav_series->execute([$usuarioid]);
...
#fav_ico{
    ...
    <nav>
        ...
        <div><img onclick="location.href='./home.php'" onmouseenter="this.src='../../assets/home-black.png'" onmouseout="this.src='../../assets/home-white.png'"src="../../assets/home-white.png" alt=""></div>
            <div><img class="intro" onclick="location.href='./movies.php'" src="../../assets/video.png" alt="Películas" title="Películas"></div>
            <div><img class="study" onclick="location.href='./series.php'" src="../../assets/series.png" alt="Series" title="Series"></div>
    ...
    <div id="container">
        ...
        <main id="pelis">
            <div id="contenedor_bienvenida">
                 <img id='favs' src="../../assets/favs.png" alt="Favoritos"> <h1 id="user_name"> <?= htmlspecialchars($nombreUsuario, ENT_QUOTES, 'UTF-8'); ?></h1>
            </div>
