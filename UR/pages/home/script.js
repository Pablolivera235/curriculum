...
(function () {
    ...
    window.abrirModalSerie = function (id) {
        ...
        let capsHtml = caps.length > 0 ? caps.map(cap => `
            <li onclick="window.abrirVideo('${cap.url}', '${cap.nombre}', ${id})">
        ...
        modalContenido.innerHTML = `
            <div class="content">
                ...
                <button id="ser_ver" onclick="window.abrirVideo('${caps[0]?.url || ''}', '${caps[0]?.nombre || ''}', ${item.id})">VER AHORA</button>
    ...
    window.abrirVideo = (url, epName, seriesId) => {
        if (!url) {
